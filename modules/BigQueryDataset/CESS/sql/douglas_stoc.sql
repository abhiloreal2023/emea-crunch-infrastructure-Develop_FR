insert into `emea-crunch-gbl-emea-{env}.d_crunch_cess_dom_{env}.t_lpd_douglas_b-o_stoc` (EAN_UPC_CODE,SOLD_QUANTITY_IN_UNITS,POS_OR_WEBSITE_CODE,INSERT_DATETIME, UPDATE_DATETIME,SHOPPING_CHANNEL,RUN_ID)
...
SELECT * FROM `emea-crunch-gbl-emea-{env}.d_crunch_cess_stg_{env}.t_lpd_douglas_b-o_stoc`
...
where EAN is null and Stoc_in_bucati_total = 'Total' and RUN_ID = '{run_id}'
),
...
Retailer_Data AS(
  ...
  where EAN IS NOT NULL and Stoc_in_bucati_total <> 'Stoc in bucati' and RUN_ID = '{run_id}'
